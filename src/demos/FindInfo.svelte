<script>
  ...
  const L = {
    zh: {
      ...
      modes: [
        { name: '憑記憶答', kind: 'warn', badge: '快，但可能過時／編造', ans: '就我所知，目前領先的大概是〔某個模型〕……', note: '這來自我的訓練記憶，停在截止日；之後出的新模型我並不知道，還可能自信地講錯。', sources: [] },
        { name: '先上網搜尋', kind: 'ok', badge: '有查證、附來源', ans: '（已搜尋）根據最近的報導與排行榜，目前討論度最高的是〔近期模型〕，各家互有領先。', note: '來源仍要自己點進去確認：是否可信、是否夠新。', sources: ['官方部落格', '科技媒體', '公開排行榜'] },
        { name: '深度研究', kind: 'ok', badge: '多來源交叉比對，產出報告', ans: '（深度研究：讀了 20+ 來源、交叉比對）整理成一份帶引用的比較：推理、成本、開源程度上各有領先者……', note: '適合值得等的大問題；日常小問題用一般搜尋就好。', sources: ['論文', '官方文件', '多家評測', '排行榜'] },
      ],
      srcLabel: '來源',
    ...
    en: {
      ...
      modes: [
        { name: 'From memory', kind: 'warn', badge: 'fast, but maybe stale / made-up', ans: 'As far as I know, the leader is probably [some model]…', note: 'This is from my training memory, frozen at a cutoff; I don’t know about newer models released since, and may state it confidently wrong.', sources: [] },
        { name: 'Search first', kind: 'ok', badge: 'grounded, with sources', ans: '(searched) Based on recent reports and leaderboards, the most-discussed is [a recent model], with different labs leading on different things.', note: 'Still open the sources yourself: are they credible, are they fresh?', sources: ['official blog', 'tech media', 'public leaderboard'] },
        { name: 'Deep research', kind: 'ok', badge: 'many sources cross-checked, a report', ans: '(deep research: read 20+ sources, cross-checked) a cited comparison: different leaders on reasoning, cost, and openness…', note: 'Best for questions worth the wait; use plain search for everyday ones.', sources: ['papers', 'official docs', 'several evals', 'leaderboards'] },
      ],
      srcLabel: 'Sources',
    ...
    ja: {
      ...
      modes: [
        { name: '記憶で答える', kind: 'warn', badge: '速いが古い／捏造の恐れ', ans: '私の知る限り、先頭はたぶん〔あるモデル〕……', note: 'これは訓練記憶で締切時点まで。以降の新モデルは知らず、自信満々に誤ることも。', sources: [] },
        { name: 'まず検索', kind: 'ok', badge: '根拠つき、出典あり', ans: '（検索済み）最近の報道とランキングでは、話題性が最も高いのは〔最近のモデル〕で、項目ごとに各社が先行。', note: '出典は自分で開いて確認を：信頼できるか、十分新しいか。', sources: ['公式ブログ', 'テック媒体', '公開ランキング'] },
        { name: '深層リサーチ', kind: 'ok', badge: '多ソース照合、レポート化', ans: '（深層リサーチ：20+ ソースを読み照合）引用つき比較：推論・コスト・オープン度で先行者が異なる……', note: '待つ価値のある大きな問い向け。日常は通常検索で十分。', sources: ['論文', '公式文書', '複数の評価', 'ランキング'] },
      ],
      srcLabel: '出典',
...
<div class="panel">
  ...
  <div class="ctl-row">
    ...
    {#each ui.modes as md, i}
      <button class="pl" class:on={m === i} onclick={() => (m = i)}>{md.name}</button>
    {/each}
  </div>
