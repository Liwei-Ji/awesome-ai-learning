<script>
  /* Ch4 · Deep Learning 逐層 —— 引導式「親手逐層推進」。
  ...
  const STAGES = [
    ['Input',   '原始照片（像素）',        '🐱', '接收原始像素——此時 AI 只看到一堆亮暗不同的點，還不知道那是什麼。'],
    ['Layer 1', '找出邊緣線條',            '✏️', '找出邊緣與線條——偵測明暗交界，把像素整理成一條條輪廓。'],
    ['Layer 2', '組成形狀',                '🔷', '把線條組成形狀——曲線、圓弧、三角形這些基本幾何開始浮現。'],
    ['Layer 3', '認出部位：耳朵、眼睛',    '👀', '認出部位——形狀被組合成有意義的零件：尖尖的耳朵、圓圓的眼睛。'],
    ['Output',  '判斷：這是貓',            '✅', '綜合判斷——把所有部位拼起來，得出結論：這是一隻貓。'],
  ];
  ...
  let reached = $derived(STAGES.map((_, i) => i <= active));
  let done = $derived(active >= LAST);
  let current = $derived(STAGES[active]);
...
<div class="panel">
  <div class="panel-h"><h3>互動：一層一層看 AI 怎麼「看」貓</h3><span class="eyebrow">★ Interactive</span></div>
  <p class="lede">
    深度學習的「深」，是指資料要<b>穿過好幾層</b>處理。每按一次「下一層」，
    看它如何把上一層的結果<b>再抽象一次</b>——從像素，一路長成「這是貓」。
  </p>
  ...
  <div class="demo-stage light">
    <div class="row">
      {#each STAGES as s, i}
        <div class="cell" class:lit={reached[i]} class:active={i === active}>
          <div class="ico">{s[2]}</div>
          <div class="mono nm">{s[0]}</div>
    ...
    <div class="whatbox" class:finished={done}>
      <span class="fieldk">{done ? '完成' : `這一層在做什麼 · 第 ${active + 1} / ${STAGES.length} 層`}</span>
      {#key active}
        <div class="whattext" in:fly={{ y: 6, duration: dur(D.base), easing: ease }}>
          <b class="tag">{current[0]}</b>{current[3]}
        </div>
      {/key}
    </div>
    ...
    <div class="guide-actions">
      {#if done}
        <button class="btn primary" disabled>✓ 完成</button>
      {:else}
        <button class="btn primary" disabled={playing} onclick={next}>▶ 下一層</button>
      {/if}
      <span class="arrow2">→</span>
      <button class="btn sm" disabled={playing} onclick={autoplay}>▶ 自動播放</button>
      <button class="btn ghost" onclick={reset}>重置</button>
    </div>
  </div>

  <p class="hint">
    越前面的層看<b>細節</b>（邊緣），越後面的層看<b>整體</b>（耳朵、臉）。每層都把前一層的結果<b>再抽象一次</b>——這就是「深度」的意思。
  </p>
</div>
