<script>
  ...
  function draw() {
    ...
    cx.fillStyle = 'rgba(20,150,140,.95)'; cx.fillText('貓', 74, 30);
    cx.fillStyle = 'rgba(224,140,20,.95)'; cx.fillText('狗', 312, 188);
    cx.font = '600 11px system-ui, -apple-system, sans-serif';
    cx.fillStyle = 'rgba(103,116,140,.95)';
    cx.fillText('分類線', 245, 125.7 - 0.32 * err - 7);
  }
  $effect(() => { v; draw(); });
</script>

<div class="panel">
  <div class="panel-h"><h3>互動：資料量越大，模型越準</h3><span class="eyebrow">★ Interactive</span></div>
  <p class="lede">
    機器學習靠<b>資料</b>找規律。先挑一個資料量，或親手拖動滑桿，看散點圖裡的<b>分類線</b>與<b>準確率</b>怎麼變。
  </p>

  <div class="demo-stage light">
    <div class="presets">
      <span class="fieldk">選一個資料量，或自己拖滑桿</span>
      <div class="pl-row">
        <button class="pl" class:on={region === 0} onclick={() => preset(0)}>少量 · 100 筆</button>
        <button class="pl" class:on={region === 1} onclick={() => preset(74)}>中等 · 3,000 筆</button>
        <button class="pl" class:on={region === 2} onclick={() => preset(100)}>大量 · 10,000 筆</button>
      </div>
    </div>

    <div class="top">
      <div class="stat-wrap">
        <span class="fieldk">準確率 Accuracy</span>
    ...
    <div class="ctl">
      <div class="lab"><span>資料量 Dataset size</span><b>{nLabel} 筆</b></div>
  ...
  <p class="hint">
    資料是機器學習的<b>養分</b>：量到位，規律才學得出來。少量資料下模型記住的是巧合；資料一多，雜訊被稀釋，
    真正的規律就浮現、分類線也<b>乾淨俐落</b>。
  </p>
</div>
